...
public class StatisticsSettingsAndWorkSheetHandlersTests
{
    ...
    [Fact] public async Task Settings_commands_update_existing_values() { var r=new Mock<ISystemSettingRepository>();var s=new SystemSetting{SettingKey="LabName",SettingValue="Old"};r.Setup(x=>x.GetByKeysAsync(It.IsAny<IReadOnlyCollection<string>>(),It.IsAny<CancellationToken>())).ReturnsAsync(new List<SystemSetting>{s});await new UpdateAccountSettingsCommandHandler(r.Object,new Mock<IUnitOfWork>().Object).Handle(new("New","EGP"),default);Assert.Equal("New",s.SettingValue);r.Verify(x=>x.Update(s),Times.Once); }

    [Fact] public async Task ManageBackup_invokes_backup_service_for_backup() { var service=new Mock<IBackupService>();service.Setup(x=>x.BackupAsync("file.bak",It.IsAny<CancellationToken>())).ReturnsAsync(new BackupResult("MasrLabDb","file.bak",DateTimeOffset.UtcNow));await new ManageBackupCommandHandler(service.Object).Handle(new(BackupOperation.Backup,"file.bak"),default);service.Verify(x=>x.BackupAsync("file.bak",It.IsAny<CancellationToken>()),Times.Once); }
    [Fact] public async Task ManageBackup_invokes_backup_service_for_restore() { var service=new Mock<IBackupService>();service.Setup(x=>x.RestoreAsync("file.bak","RESTORE",It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);await new ManageBackupCommandHandler(service.Object).Handle(new(BackupOperation.Restore,"file.bak","RESTORE"),default);service.Verify(x=>x.RestoreAsync("file.bak","RESTORE",It.IsAny<CancellationToken>()),Times.Once); }
}
