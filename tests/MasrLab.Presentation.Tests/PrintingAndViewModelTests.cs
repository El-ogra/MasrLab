...
{ [Fact] public async Task EnvelopePrinter_DelegatesToPrintService() { var service=new Mock<IPrintService>(); var payload=new EnvelopePrintDto(); await new EnvelopePrinter(service.Object).PrintAsync(payload); service.Verify(x=>x.PrintAsync(PrintReportNames.Envelope,payload,null,It.IsAny<CancellationToken>())); }
 ...
 [Fact] public void Di_ResolvesEveryReportDefinition() { var services=new ServiceCollection(); services.AddInfrastructure(new ConfigurationBuilder().AddInMemoryCollection(new Dictionary<string,string?> { ["ConnectionStrings:MasrLab"]="Server=(localdb)\\MSSQLLocalDB;Database=MasrLabTests;Trusted_Connection=True" }).Build()); using var provider=services.BuildServiceProvider(); Assert.Equal(12,provider.GetServices<IReportDefinition>().Count()); Assert.NotNull(provider.GetRequiredService<ReportDefinitionRegistry>()); } }
